  ...
  return (
    <main className="relative w-full bg-black overflow-hidden text-white" style={{ height: '100svh', minHeight: '-webkit-fill-available' }}>
      ...
      <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 4 }}>
        ...
        {/* ─── MOBILE LINES ─── */}
        {/* Vertical up — from center (50%, 45%) to top */}
        ...
        {/* Down-right — from (50%,45%) to (100%,60%) */}
        <div className="md:hidden" style={{
          position: 'absolute',
          left: 'calc(50% - 0.5px)',
          top: '45%',
      ...
      <div className="md:hidden absolute inset-0" style={{ zIndex: 3 }}>
        ...
        <div className="absolute inset-0" style={{ clipPath: 'polygon(50% 45%, 50% 0%, 0% 0%, 0% 60%, 50% 45%)' }}>
          <TiltCard className="" />
          <Link href="/process" className="absolute inset-0 z-[2] cursor-pointer" />
          <div className="absolute pointer-events-none" style={{ top: '24%', left: 20, transform: 'perspective(800px) rotateX(var(--tilt-x, 0deg)) rotateY(var(--tilt-y, 0deg)) scale(var(--tilt-scale, 1))', transformStyle: 'preserve-3d', willChange: 'transform' }}>
        ...
        <div className="absolute inset-0" style={{ clipPath: 'polygon(50% 45%, 50% 0%, 100% 0%, 100% 60%, 50% 45%)' }}>
          <TiltCard className="" />
          <Link href="/gallery" className="absolute inset-0 z-[2] cursor-pointer" />
          <div className="absolute pointer-events-none" style={{ top: '24%', right: 20, transform: 'perspective(800px) rotateX(var(--tilt-x, 0deg)) rotateY(var(--tilt-y, 0deg)) scale(var(--tilt-scale, 1))', transformStyle: 'preserve-3d', willChange: 'transform' }}>
        ...
        {/* Bottom section — SHOP */}
        <div className="absolute inset-0" style={{ clipPath: 'polygon(50% 45%, 0% 60%, 0% 100%, 100% 100%, 100% 60%, 50% 45%)' }}>
          <TiltCard className="" />
          <Link href="/shop" className="absolute inset-0 z-[2] cursor-pointer" />
